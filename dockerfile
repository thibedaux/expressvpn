...
Docker Compose
  ...
  expressvpn:
    ...
    privileged: true
HEALTHCHECK
Healthcheck is performed once every 2min. You can also add --env=DDSN=domain or --env=IP=yourIP to docker run command or in enviroment section of compose in order to perform healthcheck which will be checking if data from env variable DDNS or IP is different than ExpressVPN's IP. If you won't set any of them, by default healthcheck will return status healthy. Also, there is a possibility to add --env=BEAERER=access_token from ipinfo.io if you have an account there (free plan gives you 50k requests per month).
...
SERVER
You can choose to which location ExpressVPN should connect to by setting up SERVER=ALIAS/COUNTRY/LOCATION/SMART Check the table below for the full list of available servers to connect to. You can also check available locations from inside the container by running expressvpn list all command.
